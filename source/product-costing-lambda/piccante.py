...
def get_data_piccante(start_date = (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d"), end_date = datetime.now().strftime("%Y-%m-%d")):
    ...
    df = pd.DataFrame(
        {
            ...
            "Salame": pd.Series([0] * len(serie_data)),
    ...
    # Salame (200 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%VENTRICINA%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    pesi_salame = []
    for i, row in df_temp.iterrows():
        if row['UnitaMisura'].upper() == 'K.':
            pesi_salame.append(1)
        else:
            pesi_salame.append(pesi_salame[-1] if len(pesi_salame) > 0 else 1)
    df_temp["Peso"] = pd.Series(pesi_salame)
    df_temp['Prezzo Salame Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 50
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Salame'] = row['Prezzo Salame Pezzo']
    ...
    # Totale_CV
    df['Totale_CV'] = df['Farina'] + df['Olio'] + df['Sale'] + df['Zucchero'] + df['Lievito'] + df['Pomodoro'] + df['Mozzarella'] + df['Salame']
...
if __name__ == "__main__":
    get_data_piccante("2024-01-01","2025-10-04") 