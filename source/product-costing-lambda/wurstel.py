...
def get_data_wurstel(start_date = (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d"), end_date = datetime.now().strftime("%Y-%m-%d")):
    ...
    df = pd.DataFrame(
        {
            ...
            "Wurstel": pd.Series([0] * len(serie_data)),
    ...
    # Mozzarella (624 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    ...
    df_temp['Prezzo Mozzarella Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 16.0256410256
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Mozzarella'] = row['Prezzo Mozzarella Pezzo']

    # Wurstel (350 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%WURSTEL%%' AND UPPER(P.Nome) LIKE '%%JUPPY%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    pesi_wurstel = []
    for i, row in df_temp.iterrows():
        valore = extract_wurstel_gr(row['Nome']) * 18
        if valore:
            pesi_wurstel.append(valore)
        else:
            try:
                valore = estrai_peso_bedrock(row['Nome'])
                if valore:
                    ...
                    pesi_wurstel.append(pesi_wurstel[-1] if len(pesi_wurstel) > 0 else 1)
            except Exception as e:
                pesi_wurstel.append(pesi_wurstel[-1] if len(pesi_wurstel) > 0 else 1)
    df_temp["Peso"] = pd.Series(pesi_wurstel)
    df_temp['Prezzo Wurstel Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 0.02857142857
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Wurstel'] = row['Prezzo Wurstel Pezzo']
    ...
    # Totale_CV
    df['Totale_CV'] = df['Farina'] + df['Olio'] + df['Sale'] + df['Zucchero'] + df['Lievito'] + df['Pomodoro'] + df['Mozzarella'] + df['Wurstel']
...
if __name__ == "__main__":
    get_data_wurstel("2024-01-01", "2025-10-04")