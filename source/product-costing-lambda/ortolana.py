...
def get_data_ortolana(start_date = (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d"), end_date = datetime.now().strftime("%Y-%m-%d")):
    ...
    df = pd.DataFrame(
        {
            ...
            "Patate": pd.Series([0] * len(serie_data)),
    ...
    # Mozzarella (458 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    ...
    df_temp['Prezzo Mozzarella Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 21.8340611
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Mozzarella'] = row['Prezzo Mozzarella Pezzo']

    # Melanzane (206 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    ...
    df_temp['Prezzo Melanzane Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 48.5436893
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Melanzane'] = row['Prezzo Melanzane Pezzo']

    # Patate (294 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%PATATE%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    pesi_patate = []
    for i, row in df_temp.iterrows():
        pesi_patate.append(2.5)        
    df_temp["Peso"] = pd.Series(pesi_patate)
    df_temp['Prezzo Patate Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 34.0136054
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Patate'] = row['Prezzo Patate Pezzo']

    # Zucchine (164 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    ...
    df_temp['Prezzo Zucchine Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 60.9756098
    df_temp.sort_values("Data", inplace=True)
    ...
    df_temp['Prezzo Pomodorini Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 32.2580645
    df_temp.sort_values("Data", inplace=True)
    ...
    # Totale_CV
    df['Totale_CV'] = df['Farina'] + df['Olio'] + df['Sale'] + df['Zucchero'] + df['Lievito'] + df['Mozzarella'] + df['Melanzane'] + df['Patate'] + df['Zucchine'] + df['Pomodorini'] + df["Olio_d_oliva"]
...
if __name__ == "__main__":
    get_data_ortolana("2024-01-01","2025-10-04")