...
def get_data_grasselli(start_date = (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d"), end_date = datetime.now().strftime("%Y-%m-%d")):
    ...
    df = pd.DataFrame(
        {
            ...
            "Grasselli": pd.Series([0] * len(serie_data)),
    ...
    df_temp['Prezzo Farina Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 14.208
    ...
    df_temp['Prezzo Olio Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Litri"] / 50.12128
    df_temp.sort_values("Data", inplace=True)
    ...
    df_temp['Prezzo Sale Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 473.6
    df_temp.sort_values("Data", inplace=True)
    ...
    df_temp['Prezzo Zucchero Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 947.2
    df_temp.sort_values("Data", inplace=True)
    ...
    df_temp['Prezzo Lievito Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 473600
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Lievito'] = row['Prezzo Lievito Pezzo']

    # Grasselli (250 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%GRASSELLI%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    pesi_grasselli = []
    for i, row in df_temp.iterrows():
        if row['UnitaMisura'].upper() == 'KG':
            pesi_grasselli.append(1)
        else:
            pesi_grasselli.append(pesi_grasselli[-1] if len(pesi_grasselli) > 0 else 1)
    df_temp["Peso"] = pd.Series(pesi_grasselli)
    df_temp['Prezzo Grasselli Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 40
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Grasselli'] = row['Prezzo Grasselli Pezzo']
    ...
    # Totale_CV
    df['Totale_CV'] = df['Farina'] + df['Olio'] + df['Sale'] + df['Zucchero'] + df['Lievito'] + df['Grasselli']
...
if __name__ == "__main__":
    get_data_grasselli("2024-01-01","2025-10-04")