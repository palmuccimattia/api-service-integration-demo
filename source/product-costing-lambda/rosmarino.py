...
def get_data_rosmarino(start_date = (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d"), end_date = datetime.now().strftime("%Y-%m-%d")):
    ...
    df = pd.DataFrame(
        {
            ...
            "Rosmarino": pd.Series([0] * len(serie_data)),
    ...
    # Rosmarino (6 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%CANNAMELA ROSMARINO%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    ...
    pesi_rosmarino = []
    for i, row in df_temp.iterrows():
        valore = extract_rosmarino_kg(row['Nome'])
        if valore:
            pesi_rosmarino.append(valore)
        else:
            try:
                valore = estrai_peso_bedrock(row['Nome'])
                if valore:
                    ...
                    pesi_rosmarino.append(pesi_rosmarino[-1] if len(pesi_rosmarino) > 0 else 1)
            except Exception as e:
                pesi_rosmarino.append(pesi_rosmarino[-1] if len(pesi_rosmarino) > 0 else 1)
    df_temp["Peso"] = pd.Series(pesi_rosmarino)
    df_temp['Prezzo Rosmarino Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 1666.666666666667
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Rosmarino'] = row['Prezzo Rosmarino Pezzo']
    ...
    # Totale_CV
    df['Totale_CV'] = df['Farina'] + df['Olio'] + df['Sale'] + df['Zucchero'] + df['Lievito'] + df['Rosmarino']
...
if __name__ == "__main__":
    get_data_rosmarino("2024-01-01", "2025-10-04")