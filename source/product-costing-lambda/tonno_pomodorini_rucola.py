...
def get_data_tonnopomodorinirucola(start_date = (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d"), end_date = datetime.now().strftime("%Y-%m-%d")):
    ...
    df = pd.DataFrame(
        {
            ...
            "Tonno": pd.Series([0] * len(serie_data)),
    ...
    # Pomodorini (280 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    ...
    df_temp['Prezzo Pomodorini Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 35.7142857143
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Pomodorini'] = row['Prezzo Pomodorini Pezzo']

    # Tonno (384 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%TONNO%%' AND UPPER(P.Nome) LIKE '%%OLIO VEGETALE%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    df_temp = aggrega_quantita_prezzo_zero(df_temp)
    pesi_tonno = []
    for i, row in df_temp.iterrows():
        valore = extract_tonno_gr(row['Nome'])
        if valore:
            pesi_tonno.append(valore)
        else:
            try:
                valore = estrai_peso_bedrock(row['Nome'])
                if valore:
                    ...
                    pesi_tonno.append(pesi_tonno[-1] if len(pesi_tonno) > 0 else 1)
            except Exception as e:
                pesi_tonno.append(pesi_tonno[-1] if len(pesi_tonno) > 0 else 1)
    df_temp["Peso"] = pd.Series(pesi_tonno)
    df_temp['Prezzo Tonno Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 0.02604166667
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Tonno'] = row['Prezzo Tonno Pezzo']

    # Rucola (50 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    ...
    df_temp['Prezzo Rucola Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 0.2
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Rucola'] = row['Prezzo Rucola Pezzo']

    # Olio_d_oliva (36 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    ...
    df_temp['Prezzo Olio Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 255.5583950933
    df_temp.sort_values("Data", inplace=True)
    ...
    # Totale_CV
    df['Totale_CV'] = df['Farina'] + df['Olio'] + df['Sale'] + df['Zucchero'] + df['Lievito'] + df['Pomodoro'] + df['Pomodorini'] + df['Tonno'] + df['Rucola'] + df["Olio_d_oliva"]
...
if __name__ == "__main__":
    get_data_tonnopomodorinirucola("2024-01-01","2025-10-04")