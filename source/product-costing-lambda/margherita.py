...
def get_data_margherita(start_date = (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d"), end_date = datetime.now().strftime("%Y-%m-%d")):
    ...
    # Mozzarella (824 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    ...
    df_temp['Prezzo Mozzarella Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 12.1359223301
    df_temp.sort_values("Data", inplace=True)
    ...
    # query = f"""SELECT
    # s.product_descr,
    # SUM(s.price) AS totale_venduto
    # FROM Scontrini s
    # WHERE YEAR(s.date_time) = {actual_year - 1}
    # GROUP BY s.product_descr;
    # """
    # df_temp2 = pd.read_sql(query, con=engine)
    # # Calcolo unitario corretto del CF Generale
    # margherita_row = df_temp2[df_temp2['product_descr'].str.lower().str.strip() == 'margherita']
    ...
    if not df_temp2.empty:
        # margherita_venduto = float(margherita_row['totale_venduto'].values[0])
        ...
        if not df_temp3.empty:
            ...
            # # âš ï¸ ATTENZIONE: serve sapere quante margherite sono state vendute (qty)
            # query_qty = f"""
            # SELECT SUM(qty) as totale_qty
            # FROM Scontrini
            # WHERE LOWER(TRIM(product_descr)) = 'margherita'
            # AND YEAR(date_time) = {actual_year - 1}
            # """
            # df_qty = pd.read_sql(query_qty, con=engine)
            
            # totale_qty = df_qty['totale_qty'][0]
            
            if qty_totale and qty_totale > 0:
                # costo_unitario_cf_generale = totale_generale * margherita_venduto / df_temp2['totale_venduto'].sum() / totale_qty
    ...
    # Totale_CV
    df['Totale_CV'] = df['Farina'] + df['Olio'] + df['Sale'] + df['Zucchero'] + df['Lievito'] + df['Pomodoro'] + df['Mozzarella']
...
if __name__ == "__main__":
    get_data_margherita("2024-01-01", "2025-10-04")