...
def get_data_patate(start_date = (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d"), end_date = datetime.now().strftime("%Y-%m-%d")):
    ...
    # Totale_CV
    df['Totale_CV'] = df['Farina'] + df['Olio'] + df['Sale'] + df['Zucchero'] + df['Lievito'] + df['Patate'] + df['Mozzarella'] + df['GranaPadano']
...
if __name__ == "__main__":
    get_data_patate("2024-01-01","2025-10-04")