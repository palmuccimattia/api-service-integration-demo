...
def get_data_capricciosa(start_date = (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d"), end_date = datetime.now().strftime("%Y-%m-%d")):
    ...
    df = pd.DataFrame(
        {
            ...
            "Pomodoro": pd.Series([0] * len(serie_data)),
            "Mozzarella": pd.Series([0] * len(serie_data)),
            "Funghi": pd.Series([0] * len(serie_data)),
            "Carciofi": pd.Series([0] * len(serie_data)),
            "ProsciuttoCotto": pd.Series([0] * len(serie_data)),
            "Olive": pd.Series([0] * len(serie_data)),
    ...
    # Pomodoro (556 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%% CIRIO %%' AND UPPER(P.Nome) LIKE '%% POMODORI %%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    df_temp = aggrega_quantita_prezzo_zero(df_temp)
    pesi_pomodori = []
    for i, row in df_temp.iterrows():
        valore = extract_pomodori_kg(row['Nome'])
        if valore:
            pesi_pomodori.append(valore)
        else:
            try:
                valore = estrai_peso_bedrock(row['Nome'])
                if valore:
                    ...
                    pesi_pomodori.append(pesi_pomodori[-1] if len(pesi_pomodori) > 0 else 1)
            except Exception as e:
                pesi_pomodori.append(pesi_pomodori[-1] if len(pesi_pomodori) > 0 else 1)
    df_temp["Peso"] = pd.Series(pesi_pomodori)        
    df_temp['Prezzo Pomodoro Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 17.9856115108
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Pomodoro'] = row['Prezzo Pomodoro Pezzo']

    # Mozzarella (442 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    ...
    df_temp['Prezzo Mozzarella Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 22.6244343891
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Mozzarella'] = row['Prezzo Mozzarella Pezzo']

    # Funghi (160 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%FUNGO%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    pesi_funghi = []
    for i, row in df_temp.iterrows():
        if row['UnitaMisura'].upper() == 'KG':
            pesi_funghi.append(1)
        else:
            pesi_funghi.append(pesi_funghi[-1] if len(pesi_funghi) > 0 else 1)
    df_temp["Peso"] = pd.Series(pesi_funghi)
    df_temp['Prezzo Funghi Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 62.5
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Funghi'] = row['Prezzo Funghi Pezzo']

    # Carciofi (224 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%CARCIOFI A FETTINE%%' AND UPPER(P.Nome) LIKE '%%NAZIONALI%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    df_temp = aggrega_quantita_prezzo_zero(df_temp)
    pesi_carciofi = []
    for i, row in df_temp.iterrows():
        valore = extract_carciofi_gr(row['Nome'])
        if valore:
            pesi_carciofi.append(valore)
        else:
            try:
                valore = estrai_peso_bedrock(row['Nome'])
                if valore:
                    ...
                    pesi_carciofi.append(pesi_carciofi[-1] if len(pesi_carciofi) > 0 else 1)
            except Exception as e:
                pesi_carciofi.append(pesi_carciofi[-1] if len(pesi_carciofi) > 0 else 1)
    df_temp["Peso"] = pd.Series(pesi_carciofi)
    df_temp['Prezzo Carciofi Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 0.0446428571
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Carciofi'] = row['Prezzo Carciofi Pezzo']

    # Prosciutto cotto (104 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%PROSCIUTTO COTTO%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    pesi_prosciutto = []
    for i, row in df_temp.iterrows():
        if row['UnitaMisura'].upper() == 'KG':
            pesi_prosciutto.append(1)
        else:
            pesi_prosciutto.append(pesi_prosciutto[-1] if len(pesi_prosciutto) > 0 else 1)
    df_temp["Peso"] = pd.Series(pesi_prosciutto)
    df_temp['Prezzo Prosciutto Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 96.1538461538
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'ProsciuttoCotto'] = row['Prezzo Prosciutto Pezzo']

    # Olive (88 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%OLIVE NERE DENOCCIOLATE%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    pesi_olive = []
    for i, row in df_temp.iterrows():
        valore = extract_olive_gr(row['Nome'])
        if valore:
            pesi_olive.append(valore)
        else:
            try:
                valore = estrai_peso_bedrock(row['Nome'])
                if valore:
                    ...
                    pesi_olive.append(pesi_olive[-1] if len(pesi_olive) > 0 else 1)
            except Exception as e:
                pesi_olive.append(pesi_olive[-1] if len(pesi_olive) > 0 else 1)
    df_temp["Peso"] = pd.Series(pesi_olive)
    df_temp['Prezzo Olive Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 0.1136363636
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Olive'] = row['Prezzo Olive Pezzo']
    ...
    # Totale_CV
    df['Totale_CV'] = df['Farina'] + df['Olio'] + df['Sale'] + df['Zucchero'] + df['Lievito'] + df['Pomodoro'] + df['Mozzarella'] + df['Funghi'] + df['Carciofi'] + df['ProsciuttoCotto'] + df['Olive']
...
if __name__ == "__main__":
    get_data_capricciosa("2024-01-01","2025-10-04")