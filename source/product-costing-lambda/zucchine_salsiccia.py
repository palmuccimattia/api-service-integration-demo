...
def get_data_zucchinesalsiccia(start_date = (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d"), end_date = datetime.now().strftime("%Y-%m-%d")):
    ...
    df_temp['Prezzo Salsiccia Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 41.6666667
    df_temp.sort_values("Data", inplace=True)
    ...
    # Totale_CV
    df['Totale_CV'] = df['Farina'] + df['Olio'] + df['Sale'] + df['Zucchero'] + df['Lievito'] + df['Mozzarella'] + df['Zucchine'] + df['Salsiccia'] + df["Olio_d_oliva"]
...
if __name__ == "__main__":
    get_data_zucchinesalsiccia("2024-01-01","2025-10-04")