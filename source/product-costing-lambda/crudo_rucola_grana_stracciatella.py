...
def get_data_crudorucolagranastracciatella(start_date = (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d"), end_date = datetime.now().strftime("%Y-%m-%d")):
    ...
    df = pd.DataFrame(
        {
            ...
            "ProsciuttoCrudo": pd.Series([0] * len(serie_data)),
            "Rucola": pd.Series([0] * len(serie_data)),
            "GranaPadanoExtra": pd.Series([0] * len(serie_data)),
            "Pomodorini": pd.Series([0] * len(serie_data)),
            "Olio_d_oliva": pd.Series([0] * len(serie_data)),
            "AcetoBalsamico": pd.Series([0] * len(serie_data)),
    ...
    # Stracciatella (500 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    ...
    df_temp['Prezzo Stracciatella Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 20
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Stracciatella'] = row['Prezzo Stracciatella Pezzo']

    # Prosciutto crudo (200 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%CRUDO%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    pesi_crudo = []
    for i, row in df_temp.iterrows():
        if row['UnitaMisura'].upper() == 'KG':
            ...
            pesi_crudo.append(pesi_crudo[-1] if len(pesi_crudo) > 0 else 1)
    df_temp["Peso"] = pd.Series(pesi_crudo)
    df_temp['Prezzo Crudo Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 50
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'ProsciuttoCrudo'] = row['Prezzo Crudo Pezzo']

    # Rucola (150 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    ...
    df_temp['Prezzo Rucola Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 0.0666666667
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Rucola'] = row['Prezzo Rucola Pezzo']

    # Grana Padano Extra (100 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%GRANA PADANO EXTRA%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    pesi_grana = []
    ...
    df_temp['Prezzo Grana Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 100
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'GranaPadanoExtra'] = row['Prezzo Grana Pezzo']

    # Pomodorini (300 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    ...
    df_temp['Prezzo Pomodorini Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 33.3333333
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Pomodorini'] = row['Prezzo Pomodorini Pezzo']

    # Olio_d_oliva (30 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    ...
    df_temp['Prezzo Olio Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 306.670074
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], "Olio_d_oliva"] = row["Prezzo Olio Pezzo"]

    # Aceto Balsamico (20 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%BALSAMICO%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    pesi_balsamico = []
    for i, row in df_temp.iterrows():
        pesi_balsamico.append(600)
    df_temp["Peso"] = pd.Series(pesi_balsamico)
    df_temp['Prezzo Balsamico Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 0.5
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'AcetoBalsamico'] = row['Prezzo Balsamico Pezzo']
    ...
    # Totale_CV
    df['Totale_CV'] = df['Farina'] + df['Olio'] + df['Sale'] + df['Zucchero'] + df['Lievito'] + df['Stracciatella'] + df['ProsciuttoCrudo'] + df['Rucola'] + df['GranaPadanoExtra'] + df['Pomodorini'] + df["Olio_d_oliva"] + df["AcetoBalsamico"]
...
if __name__ == "__main__":
    get_data_crudorucolagranastracciatella("2024-01-01","2025-10-04")