...
def get_data_tartufata(start_date = (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d"), end_date = datetime.now().strftime("%Y-%m-%d")):
    ...
    df = pd.DataFrame(
        {
            ...
            "Stracciatella": pd.Series([0] * len(serie_data)),
            "CremaTartufo": pd.Series([0] * len(serie_data)),
            "Carciofi": pd.Series([0] * len(serie_data)),
            "FunghiMisti": pd.Series([0] * len(serie_data)),
            "Rucola": pd.Series([0] * len(serie_data)),
    ...
    # Stracciatella (350 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%STRACCIATELLA%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    pesi_stracciatella = []
    for i, row in df_temp.iterrows():
        if row['UnitaMisura'].upper() == 'KG':
            pesi_stracciatella.append(1)
        else:
            pesi_stracciatella.append(pesi_stracciatella[-1] if len(pesi_stracciatella) > 0 else 1)
    df_temp["Peso"] = pd.Series(pesi_stracciatella)
    df_temp['Prezzo Stracciatella Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 28.5714286
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Stracciatella'] = row['Prezzo Stracciatella Pezzo']

    # Crema Tartufo (100 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%TARTUFATA%%' AND UPPER(P.Nome) LIKE '%%800%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    ...
    pesi_tartufo = []
    for i, row in df_temp.iterrows():
        valore = extract_tartufo_gr(row['Nome'])
        if valore:
            pesi_tartufo.append(valore)
        else:
            try:
                valore = estrai_peso_bedrock(row['Nome'])
                if valore:
                    ...
                    pesi_tartufo.append(pesi_tartufo[-1] if len(pesi_tartufo) > 0 else 1)
            except Exception as e:
                pesi_tartufo.append(pesi_tartufo[-1] if len(pesi_tartufo) > 0 else 1)
    df_temp["Peso"] = pd.Series(pesi_tartufo)
    df_temp['Prezzo Tartufo Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 0.1
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'CremaTartufo'] = row['Prezzo Tartufo Pezzo']

    # Carciofi (378 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    ...
    df_temp['Prezzo Carciofi Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 0.0264550265
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Carciofi'] = row['Prezzo Carciofi Pezzo']

    # Funghi misti (314 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%MISTO FUNGHI%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    pesi_mistofunghi = []
    for i, row in df_temp.iterrows():
        valore = extract_mistofunghi_gr(row['Nome'])
        if valore:
            pesi_mistofunghi.append(valore)
        else:
            try:
                valore = estrai_peso_bedrock(row['Nome'])
                if valore:
                    ...
                    pesi_mistofunghi.append(pesi_mistofunghi[-1] if len(pesi_mistofunghi) > 0 else 1)
            except Exception as e:
                pesi_mistofunghi.append(pesi_mistofunghi[-1] if len(pesi_mistofunghi) > 0 else 1)
    df_temp["Peso"] = pd.Series(pesi_mistofunghi)
    df_temp['Prezzo Funghi Misti Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 0.0318471338
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'FunghiMisti'] = row['Prezzo Funghi Misti Pezzo']

    # Rucola (20 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%RUCOLA GR.200%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    pesi_rucola = []
    for i, row in df_temp.iterrows():
        pesi_rucola.append(200)
    df_temp["Peso"] = pd.Series(pesi_rucola)
    df_temp['Prezzo Rucola Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 0.5
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Rucola'] = row['Prezzo Rucola Pezzo']
    ...
    # Totale_CV
    df['Totale_CV'] = df['Farina'] + df['Olio'] + df['Sale'] + df['Zucchero'] + df['Lievito'] + df['Stracciatella'] + df['CremaTartufo'] + df['Carciofi'] + df['FunghiMisti'] + df['Rucola']
...
if __name__ == "__main__":
    get_data_tartufata("2024-01-01","2025-10-04")