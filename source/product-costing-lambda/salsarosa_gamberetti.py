...
def get_data_salsarosagamberetti(start_date = (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d"), end_date = datetime.now().strftime("%Y-%m-%d")):
    ...
    df = pd.DataFrame(
        {
            ...
            "Maionese": pd.Series([0] * len(serie_data)),
            "Ketchup": pd.Series([0] * len(serie_data)),
            "Pomodorini": pd.Series([0] * len(serie_data)),
            "Rucola": pd.Series([0] * len(serie_data)),
            "Gamberetti": pd.Series([0] * len(serie_data)),
    ...
    # Maionese (296 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%MAIONESE%%' AND UPPER(P.Nome) LIKE '%%ARCO%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    pesi_maionese = []
    for i, row in df_temp.iterrows():
        pesi_maionese.append(5)
    df_temp["Peso"] = pd.Series(pesi_maionese)
    df_temp['Prezzo Maionese Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 33.7837838
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Maionese'] = row['Prezzo Maionese Pezzo']

    # Ketchup (40 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%TOMATO KETCHUP%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    pesi_ketchup = []
    for i, row in df_temp.iterrows():
        pesi_ketchup.append(962.5)
    df_temp["Peso"] = pd.Series(pesi_ketchup)
    df_temp['Prezzo Ketchup Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 0.25
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Ketchup'] = row['Prezzo Ketchup Pezzo']

    # Pomodorini (364 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    ...
    df_temp['Prezzo Pomodorini Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 27.4725275
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Pomodorini'] = row['Prezzo Pomodorini Pezzo']

    # Rucola (34 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    ...
    df_temp['Prezzo Rucola Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 0.294117647
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Rucola'] = row['Prezzo Rucola Pezzo']

    # Gamberetti (250 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%GAMBERETTI%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    pesi_gamberetti = []
    for i, row in df_temp.iterrows():
        pesi_gamberetti.append(0.9)
    df_temp["Peso"] = pd.Series(pesi_gamberetti)
    df_temp['Prezzo Gamberetti Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 40
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Gamberetti'] = row['Prezzo Gamberetti Pezzo']

    # Olive (80 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    ...
    df_temp['Prezzo Olive Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 0.125
    df_temp.sort_values("Data", inplace=True)
    ...
    # Totale_CV
    df['Totale_CV'] = df['Farina'] + df['Olio'] + df['Sale'] + df['Zucchero'] + df['Lievito'] + df['Maionese'] + df['Ketchup'] + df['Pomodorini'] + df['Rucola'] + df['Gamberetti'] + df['Olive']
...
if __name__ == "__main__":
    get_data_salsarosagamberetti("2024-01-01","2025-10-04")