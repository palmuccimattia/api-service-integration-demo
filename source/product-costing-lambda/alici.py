...
def get_data_alici(start_date = (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d"), end_date = datetime.now().strftime("%Y-%m-%d")):
    ...
    df = pd.DataFrame(
        {
            ...
            "Alici": pd.Series([0] * len(serie_data)),
    ...
    # Mozzarella (700 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    ...
    df_temp['Prezzo Mozzarella Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 14.2857142857
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Mozzarella'] = row['Prezzo Mozzarella Pezzo']

    # Alici (90 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%BAULETTO%%' AND UPPER(P.Nome) LIKE '%%ELITE%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    pesi_alici = []
    for i, row in df_temp.iterrows():
        valore = extract_alici_gr(row['Nome'])
        if valore:
            pesi_alici.append(valore)
        else:
            try:
                valore = estrai_peso_bedrock(row['Nome'])
                if valore:
                    ...
                    pesi_alici.append(pesi_alici[-1] if len(pesi_alici) > 0 else 1)
            except Exception as e:
                pesi_alici.append(pesi_alici[-1] if len(pesi_alici) > 0 else 1)
    df_temp["Peso"] = pd.Series(pesi_alici)
    df_temp['Prezzo Alici Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 0.11111111111
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Alici'] = row['Prezzo Alici Pezzo']
    ...
    # Totale_CV
    df['Totale_CV'] = df['Farina'] + df['Olio'] + df['Sale'] + df['Zucchero'] + df['Lievito'] + df['Pomodoro'] + df['Mozzarella'] + df['Alici']
...
if __name__ == "__main__":
    get_data_alici("2024-01-01","2025-10-04")