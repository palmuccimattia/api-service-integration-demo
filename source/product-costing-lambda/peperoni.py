...
def get_data_peperoni(start_date = (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d"), end_date = datetime.now().strftime("%Y-%m-%d")):
    ...
    df = pd.DataFrame(
        {
            ...
            "Peperoni": pd.Series([0] * len(serie_data)),
    ...
    # Peperoni (533 gr per 10 pezzi)
    query = """
    SELECT P.*, O.Data
    FROM api_connector_demo.Prodotti P
    JOIN api_connector_demo.Ordini O ON P.IdOrdine = O.IdOrdine
    WHERE UPPER(P.Nome) LIKE '%%PEPERONE%%';
    """
    df_temp = pd.read_sql(query, con=engine)
    df_temp['Data'] = pd.to_datetime(df_temp['Data'])
    pesi_peperoni = []
    for i, row in df_temp.iterrows():
        if row['UnitaMisura'].upper() == 'KG':
            pesi_peperoni.append(1)
        else:
            pesi_peperoni.append(pesi_peperoni[-1] if len(pesi_peperoni) > 0 else 1)
    df_temp["Peso"] = pd.Series(pesi_peperoni)
    df_temp['Prezzo Peperoni Pezzo'] = df_temp["PrezzoTotale"] / df_temp["Quantita"] / df_temp["Peso"] / 18.7617260787
    df_temp.sort_values("Data", inplace=True)

    for index, row in df_temp.iterrows():
        df.loc[df['Data'] >= row['Data'], 'Peperoni'] = row['Prezzo Peperoni Pezzo']
    ...
    # Totale_CV
    df['Totale_CV'] = df['Farina'] + df['Olio'] + df['Sale'] + df['Zucchero'] + df['Lievito'] + df['Pomodoro'] + df['Mozzarella'] + df['Peperoni']
...
if __name__ == "__main__":
    get_data_peperoni("2024-01-01","2025-10-04") 